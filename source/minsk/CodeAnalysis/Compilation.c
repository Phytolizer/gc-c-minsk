...
static struct Compilation* compilation_new_continued(
    struct Compilation* previous,
    struct SyntaxTree* syntax_tree);
...
struct Compilation* compilation_continue_with(
    ...
    struct VariableStore* variables)
{
       ...
       ++i)
  {
    LIST_PUSH(
        diagnostics,
        compilation->syntax->diagnostics->diagnostics->data[i]);
  }
...
struct BoundGlobalScope* compilation_get_global_scope(
    struct Compilation* compilation)
{
  if (compilation->global_scope == NULL)
  {
    ...
    {
      previous_global_scope
          = compilation_get_global_scope(compilation->previous);
    }
    compilation->global_scope
        = bind_global_scope(previous_global_scope, compilation->syntax->root);
  }
