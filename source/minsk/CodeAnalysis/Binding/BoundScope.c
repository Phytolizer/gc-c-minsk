...
struct VariableSymbol** bound_scope_try_lookup(
    struct BoundScope* scope,
    sds name)
{
  struct BoundScopeVariableEntry* entry = bound_scope_variables_find_entry(
      scope->variables->entries,
      scope->variables->num_entries,
      name);
    ...
    struct VariableSymbol* variable)
{
  ...
  if (scope->variables->num_entries == 0
      || (double)scope->variables->count / (double)scope->variables->num_entries
          > 0.75)
  {
    long old_num_entries = scope->variables->num_entries;
    scope->variables->num_entries = (scope->variables->num_entries == 0)
        ? 8
        : scope->variables->num_entries * 2;
    struct BoundScopeVariableEntry* new_entries = mc_malloc(
        scope->variables->num_entries * sizeof(struct BoundScopeVariableEntry));
    for (long i = 0; i < old_num_entries; ++i)
    {
      struct BoundScopeVariableEntry* entry = bound_scope_variables_find_entry(
          new_entries,
          scope->variables->num_entries,
          scope->variables->entries[i].name);
      entry->name = scope->variables->entries[i].name;
      entry->value = scope->variables->entries[i].value;
    }
    scope->variables->entries = new_entries;
    entry = bound_scope_variables_find_entry(
        scope->variables->entries,
        scope->variables->num_entries,
        variable->name);
  }
...
struct VariableSymbolList* bound_scope_get_declared_variables(
    struct BoundScope* scope)
{
  struct VariableSymbolList* variables
      = mc_malloc(sizeof(struct VariableSymbolList));
  LIST_INIT(variables);
...
static struct BoundScopeVariables* bound_scope_variables_new(void)
{
  struct BoundScopeVariables* variables
      = mc_malloc(sizeof(struct BoundScopeVariables));
  variables->num_entries = INITIAL_SCOPE_SIZE;
  variables->entries = mc_malloc(
      variables->num_entries * sizeof(struct BoundScopeVariableEntry));
  variables->count = 0;
  return variables;
}
