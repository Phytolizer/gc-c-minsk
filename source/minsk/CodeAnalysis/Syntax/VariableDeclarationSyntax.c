    ...
    struct ExpressionSyntax* initializer)
{
  struct VariableDeclarationSyntax* syntax
      = mc_malloc(sizeof(struct VariableDeclarationSyntax));
  statement_syntax_init(
      &syntax->base,
      STATEMENT_SYNTAX_KIND_VARIABLE_DECLARATION_SYNTAX);
...
enum SyntaxKind variable_declaration_syntax_get_kind(
    struct VariableDeclarationSyntax* syntax)
{
  (void)syntax;
...
struct SyntaxNodeList* variable_declaration_syntax_get_children(
    struct VariableDeclarationSyntax* syntax)
{
