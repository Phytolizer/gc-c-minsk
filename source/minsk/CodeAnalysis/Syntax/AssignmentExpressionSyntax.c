    ...
    struct ExpressionSyntax* expression)
{
  struct AssignmentExpressionSyntax* syntax
      = mc_malloc(sizeof(struct AssignmentExpressionSyntax));
  expression_syntax_init(
      (struct ExpressionSyntax*)syntax,
      EXPRESSION_SYNTAX_KIND_ASSIGNMENT_EXPRESSION_SYNTAX);
...
enum SyntaxKind assignment_expression_syntax_get_kind(
    struct AssignmentExpressionSyntax* syntax)
{
  (void)syntax;
