...
static void pretty_print(FILE *stream, struct SyntaxNode *node, sds indent, bool is_last);
...
enum SyntaxKind syntax_node_get_kind(struct SyntaxNode *node)
{
    ...
    case SYNTAX_NODE_KIND_COMPILATION_UNIT:
        return compilation_unit_syntax_get_kind((struct CompilationUnitSyntax *)node);
...
struct SyntaxNodeList *syntax_node_get_children(struct SyntaxNode *node)
{
    ...
    case SYNTAX_NODE_KIND_COMPILATION_UNIT:
        return compilation_unit_syntax_get_children((struct CompilationUnitSyntax *)node);
    case SYNTAX_NODE_KIND_ELSE_CLAUSE:
...
struct TextSpan *syntax_node_get_span(struct SyntaxNode *node)
{
    ...
    struct TextSpan *last = syntax_node_get_span(children->data[children->length - 1]);
...
static void pretty_print(FILE *stream, struct SyntaxNode *node, sds indent, bool is_last)
{
    ...
    if (is_to_stdout)
    {
        fprintf(stream, "%s", node->kind == SYNTAX_NODE_KIND_TOKEN ? "\x1b[34m" : "\x1b[36m");
    }
    ...
    if (node->kind == SYNTAX_NODE_KIND_TOKEN && ((struct SyntaxToken *)node)->value->kind != OBJECT_KIND_NULL)
    {
        fprintf(stream, " %s", object_to_string(((struct SyntaxToken *)node)->value));
    }
    fprintf(stream, "\n");
