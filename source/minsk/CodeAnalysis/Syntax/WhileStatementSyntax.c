    ...
    struct StatementSyntax* body)
{
  struct WhileStatementSyntax* syntax
      = mc_malloc(sizeof(struct WhileStatementSyntax));
  statement_syntax_init(
      &syntax->base,
      STATEMENT_SYNTAX_KIND_WHILE_STATEMENT_SYNTAX);
...
enum SyntaxKind while_statement_syntax_get_kind(
    struct WhileStatementSyntax* syntax)
{
  (void)syntax;
...
struct SyntaxNodeList* while_statement_syntax_get_children(
    struct WhileStatementSyntax* syntax)
{
