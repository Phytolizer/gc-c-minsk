    ...
    struct SyntaxToken* close_brace_token)
{
  struct BlockStatementSyntax* syntax
      = mc_malloc(sizeof(struct BlockStatementSyntax));
  statement_syntax_init(
      (struct StatementSyntax*)syntax,
      STATEMENT_SYNTAX_KIND_BLOCK_STATEMENT_SYNTAX);
...
enum SyntaxKind block_statement_syntax_get_kind(
    struct BlockStatementSyntax* syntax)
{
  (void)syntax;
...
struct SyntaxNodeList* block_statement_syntax_get_children(
    struct BlockStatementSyntax* syntax)
{
