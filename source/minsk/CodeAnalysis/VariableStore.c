...
struct VariableStore* variable_store_new(void)
{
  ...
  memset(
      store->entries,
      0,
      store->num_entries * sizeof(struct VariableStoreEntry));
  store->count = 0;
  return store;
}
    ...
    struct Object* value)
{
  if ((double)store->count / (double)store->num_entries > 0.75)
  {
    ...
    for (long i = 0; i < old_num_entries; ++i)
    {
      ...
      {
        struct VariableStoreEntry* entry = find_entry_for(
            new_entries,
            store->num_entries,
            store->entries[i].symbol);
  ...
  struct VariableStoreEntry* entry
      = find_entry_for(store->entries, store->num_entries, symbol);
  entry->symbol = symbol;
  entry->value = value;
}
    ...
    struct VariableSymbol* symbol)
{
  ...
  hash %= num_entries;
  while (entries[hash].symbol != NULL
         && sdscmp(entries[hash].symbol->name, symbol->name) != 0)
  {
