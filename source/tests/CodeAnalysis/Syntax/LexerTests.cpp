...
BasicTokenList* get_dynamic_tokens()
{
    ...
    LIST_PUSH(dynamic_tokens, ((BasicToken){SYNTAX_KIND_IDENTIFIER_TOKEN, sdsnew("a")}));
    LIST_PUSH(dynamic_tokens, ((BasicToken){SYNTAX_KIND_IDENTIFIER_TOKEN, sdsnew("abc")}));
...
static inline bool requires_separator(SyntaxKind t1kind, SyntaxKind t2kind)
{
    ...
    if (t1kind == SYNTAX_KIND_IDENTIFIER_TOKEN && t2kind == SYNTAX_KIND_IDENTIFIER_TOKEN)
    {
        return true;
    }
    ...
    if (t1kind == SYNTAX_KIND_BANG_TOKEN &&
        (t2kind == SYNTAX_KIND_EQUALS_TOKEN || t2kind == SYNTAX_KIND_EQUALS_EQUALS_TOKEN))
    {
        return true;
    }
...
TEST_SUITE("Lexer")
{
    TEST_CASE("tests all token types")
    {
        ...
        {
            std::cerr << "untested: " << SYNTAX_KINDS[untested_token_kinds->data[i]] << std::endl;
        }
    }
    ...
    TEST_CASE("lexes token pairs")
    {
        for (auto& test : get_token_pairs())
        {
            ...
            CHECK(std::string{SYNTAX_KINDS[tokens->data[0]->kind]} == std::string{SYNTAX_KINDS[test.first.kind]});
            CHECK(std::string{tokens->data[0]->text} == std::string{test.first.text});

            CHECK(std::string{SYNTAX_KINDS[tokens->data[1]->kind]} == std::string{SYNTAX_KINDS[test.second.kind]});
    ...
    TEST_CASE("lexes token pairs with separator")
    {
        for (long i = 0; i < TOKEN_PAIRS_WITH_SEPARATOR->length; ++i)
        {
            ...
            CHECK(std::string{SYNTAX_KINDS[tokens->data[0]->kind]} == std::string{SYNTAX_KINDS[test.t1.kind]});
            CHECK(std::string{tokens->data[0]->text} == std::string{test.t1.text});

            REQUIRE(tokens->length > 1);

            CHECK(std::string{SYNTAX_KINDS[tokens->data[1]->kind]} == std::string{SYNTAX_KINDS[test.separator.kind]});
            CHECK(std::string{tokens->data[1]->text} == std::string{test.separator.text});

            REQUIRE(tokens->length > 2);

            CHECK(std::string{SYNTAX_KINDS[tokens->data[2]->kind]} == std::string{SYNTAX_KINDS[test.t2.kind]});
