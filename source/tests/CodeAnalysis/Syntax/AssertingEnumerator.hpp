...
AssertingEnumerator::AssertingEnumerator(SyntaxNode* root)
    : flattened(flatten(root)), enumerator(flattened.begin())
{
}
...
void AssertingEnumerator::assert_node(SyntaxKind kind)
{
  ...
  CHECK(
      std::string{SYNTAX_KINDS[syntax_node_get_kind(*enumerator)]}
      == std::string{SYNTAX_KINDS[kind]});
  ++enumerator;
}
